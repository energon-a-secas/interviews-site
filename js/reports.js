...
export function generateInternalReport() {
  ...
  md += '\n---\n\n## Anti-Cheat Signals\n\n';
  md += '**AI-assist risk:** ' + risk.label + '  \n\n';
  md += '| Observation | Rating | Score |\n|-------------|--------|-------|\n';
  CHEAT_NAMES.forEach(name => {
    md += '| ' + QUESTION_TEXT[name] + ' | ' + getLabel(name) + ' | ' + getVal(name) + '/2 |\n';
  });
  md += '\n*A single tell means little; several red flags together are the real signal.*\n';
  if (risk.label !== 'Low') {
    md += '\n**Recommendation:** Consider verifying response authenticity before client exposure — for example, a follow-up live interview with camera on and unscripted curveball questions.\n';
  }

  const cheatNotes = document.getElementById('cheat-notes').value.trim();
  if (cheatNotes) {
    md += '\n---\n\n## Anti-Cheat Notes\n\n' + cheatNotes + '\n';
  }
